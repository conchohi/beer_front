            ...
            Janus.init({
              ...
              callback: function () {
                ...
                janus = new Janus({
                  ...
                  destroyed: function () {
                    setMessage("방장에 의해 화상채팅이 종료 되었습니다.");
                    setOpenModal(true);
                  },
                });
              },
        ...
        try {
          if (remoteVideoRef.current[remoteFeed.rfindex] && remoteFeed.stream) {
            if ("srcObject" in remoteVideoRef.current[remoteFeed.rfindex]) {
              remoteVideoRef.current[remoteFeed.rfindex].srcObject =
                remoteFeed.stream;
            } else {
              remoteVideoRef.current[remoteFeed.rfindex].src =
                URL.createObjectURL(remoteFeed.stream);
            }
          }
        } catch (error) {
          console.error("Error attaching stream to element:", error);
          ...
          <div className="w-full flex flex-wrap items-start">
            <div
              className={
                ...
                (participantList.length <= 3 ? "w-1/2" : "w-1/3")
              }
            >
