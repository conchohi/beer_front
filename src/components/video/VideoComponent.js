        ...
        janus.attach(
            {
                ...
                onremotestream: function (stream) {
                    ...
                    let imgElement = videoElement?.nextSibling;
                    try {
                        if (videoElement && remoteFeed.stream) {
                            if ('srcObject' in videoElement) {
                                videoElement.srcObject = remoteFeed.stream;
                            } else {
                                videoElement.src = URL.createObjectURL(remoteFeed.stream);
                            }
                            videoElement.classList.remove('hidden');
                            imgElement?.classList.add('hidden')
                        }
    ...
    return (
        <>
            ...
            <div className="w-full flex flex-col md:flex-row">
                <div className="w-full md:w-3/4 flex flex-col flex-wrap">
                    ...
                    <div className="w-full flex flex-wrap items-start">
                        {currentGame === '고요속의외침' ? (
                            <>
                                ...
                                <div className="flex flex-wrap w-full">
                                    {participantList.map((participant) => {
                                        if (participant.nickname !== currentTurn) {
                                            return (
                                                <div key={participant.nickname} className={"flex flex-col justify-center items-center text-center p-3 w-full" + (participantList.length <= 4 ? " md:w-1/2" : " md:w-1/3")}>
                                                    <div className="w-full bg-black border-2 border-white rounded-xl">
                                                        <div className="relative">
                                                            {(master === participant.nickname && <FaCrown className="text-yellow-500 absolute right-8 top-5 z-10" size="50" />)}
                                                            <div className="pb-[56.25%] h-0 relative">
                            ...
                            <>
                                {/* 본인의 비디오 화면 */}
                                <div className={"flex flex-col justify-center rounded-lg items-center text-center p-6 w-full" + (participantList.length <= 4 ? " md:w-1/2" : " md:w-1/3")}>
                                    <div className="w-full bg-black border-2 border-white rounded-xl">
                                        <div className="relative">
                                ...
                                {participantList.map((participant) => {
                                    ...
                                    return (
                                        <div key={participant.nickname} className={"flex flex-col justify-center items-center text-center p-6 w-full" + (participantList.length <= 4 ? " md:w-1/2" : " md:w-1/3")}>
                                            <div className="w-full bg-black border-2 border-white rounded-xl">
                                                <div className="relative">
                                                    {(master === participant.nickname && <FaCrown className="text-yellow-500 absolute right-8 top-5 z-10" size="50" />)}
                                                    <div className="pb-[56.25%] h-0 relative">
                ...
                <div className="flex flex-col md:w-1/4 px-5 mt-8">
                    <Chat
                        nickname={nickname}
                        roomNo={roomNo}
