...
var defaultExtension = {
	...
	init: function () {
		...
		// 확장에서 오는 메시지를 처리하는 이벤트 리스너를 설정합니다.
...
Janus.setDefaultDependencies = function (deps) {
...
Janus.init = function(options) {
	...
	if(Janus.initDone) {
		...
		var usedDependencies = options.dependencies || Janus.setDefaultDependencies();
		...
		Janus.listDevices = function(callback, config) {
			// 콜백이 함수인지 확인하고, 아니면 빈 함수로 설정합니다.
			callback = (typeof callback == "function") ? callback : Janus.noop;
		
			// 설정 값이 없으면 기본값으로 오디오와 비디오를 모두 true로 설정합니다.
			if (config == null) config = { audio: true, video: true };
		
			// getUserMedia가 사용 가능한지 확인합니다.
			if(Janus.isGetUserMediaAvailable()) {
				// 사용자의 미디어 장치에 접근하여 스트림을 얻습니다.
				navigator.mediaDevices.getUserMedia(config)
				.then(function(stream) {
					// 사용 가능한 장치를 나열합니다.
					navigator.mediaDevices.enumerateDevices().then(function(devices) {
						Janus.debug(devices);
						// 콜백 함수를 호출하여 장치 목록을 전달합니다.
						callback(devices);
						// 더 이상 필요 없는 스트림을 중지합니다.
						Janus.stopAllTracks(stream);
					});
				})
				.catch(function(err) {
					Janus.error(err);
					// 오류가 발생하면 빈 배열을 콜백으로 반환합니다.
					callback([]);
				});
			} else {
				Janus.warn("navigator.mediaDevices unavailable");
				// mediaDevices가 사용 불가능하면 빈 배열을 콜백으로 반환합니다.
				callback([]);
			}
		};
		//미디어 스트림을 비디오 요소에 연결
		...
		//한 비디오 요소의 미디어 스트림을 다른 비디오 요소에 재연결
		...
		//IOS 여부 확인 후 ios라면 pagehide 이벤트 사용, 아니면 beforeunload 이벤트 사용
		window.addEventListener(eventName, function(event) {
			Janus.log("Closing window");
			//창을 닫거나 페이지를 떠날 때(beforeunload 또는 pagehide 이벤트) Janus 세션을 파기합니다.
		...
		// WebRTC 애플리케이션의 브라우저 호환성을 높이고, 특정 기능이 지원되는지 확인하여 적절한 처리를 할 수 있도록 합니다.
		//Janus.webRTCAdapter.browserDetails.browser가 safari이고 버전이 605 이상인지 확인합니다.
		...
		//브라우저 및 버전 확인:Firefox 버전 59 이상, Chrome 버전 72 이상에서 Unified Plan이 지원되는지 확인합니다.
		//Transceiver 지원 확인:RTCRtpTransceiver가 존재하고 currentDirection 프로퍼티가 있는지 확인합니다.없다면 Unified Plan을 지원하지 않는 것으로 간주합니다.
...
function Janus(gatewayCallbacks) {
	//Janus 생성 시 success 랑 error랑 destroyed 함수를 넣어야 됨
	...
	//Google이 제공하는 공용 STUN 서버 중 하나입니다. WebRTC 애플리케이션에서 사용자가 서로 직접 통신할 수 있도록 도움을 줍니다. 클라이언트는 이 STUN 서버에 연결하여 공용 IP 주소 및 포트를 확인하고, 이 정보를 사용하여 피어 간의 직접 연결을 설정합니다.
