    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-gray-800 text-gray-200 rounded-lg w-11/12 max-w-2xl p-6 relative">
        ...
        <div className="flex items-center mb-4">
          {/* 유저이미지 */}
          <CgProfile className="mr-2 text-pink-500"/>
        {/* {user.profileImage ? 
        (<img src="" alt="Profile" className="w-8 h-8 rounded-full mr-2" />) 
        : (<CgProfile className="mr-2 text-blue-500" size={24} />)} */}

        {/* 유저닉네임 */}
        <div className="text-gray-200 mr-2">햄토리</div>
        <input
          type="text"
          placeholder="댓글을 입력해주세요"
          className="flex-grow p-2 rounded-lg bg-gray-600 text-pink-500"
          // value={newComment}
          // onChange={(e) => setNewComment(e.target.value)}
        />
        <button className="px-4 py-2 bg-pink-500 text-white rounded-lg">등록</button>
        </div>
